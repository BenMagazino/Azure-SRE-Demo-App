# Zava Learning — SRE Agent configuration
...
agent with the **public Azure MCP SRE Agent tools** (`@azure/mcp@3.0.0-beta.16`+) and the
supported SRE Agent data-plane APIs used by the Azure portal.
...
| `skills/connectivity-triage/SKILL.md` | Edge/network incident runbook (App Gateway → NSG → ACA LB → APIs) | data-plane skill PUT |
| `skills/performance-investigation/SKILL.md` | App-tier incident runbook | data-plane skill PUT |
| `skills/rbac-audit/SKILL.md` | Read-only least-privilege audit | data-plane skill PUT |
| `skills/pr-delivery/SKILL.md` | Single owner of GitHub PR creation for an IaC or code fix; applies after live mitigation, and the PR it produces is referenced by `servicenow-change-management` | data-plane skill PUT |
| `skills/rca-analysis/SKILL.md` | RCA narrative — timeline, 5-Whys, contributing factors | data-plane skill PUT |
| `skills/evidence-before-after/SKILL.md` | Class-aware visual evidence — before/after path diagram for connectivity/config faults, time-series charts for performance/availability faults, plus a delta table | data-plane skill PUT |
| `skills/recommendations-next-steps/SKILL.md` | Prioritized preventive/detective/process actions with owners + dates | data-plane skill PUT |
...
| `incident-filter.json` | Symptom-keyed response routing scoped by an environment-specific alert-title prefix and the exact Log Analytics workspace (`titleContains: Zava-<environment>-` → default agent, autonomous, merging disabled) | incident filter create |
...
| `../tools/CreateServiceNowChangeRequest/` `../tools/UploadServiceNowAttachment/` | Custom ServiceNow Python function tools (referenced by the `servicenow-change-management` skill) | data-plane tool PUT |
...
- `@@ENVIRONMENT@@` — owning azd environment name used in alert titles and response-plan matching
...
4. **Secrets in env (never committed)** — see `.env.sample`:
...
`.env`, then creates the connectors, registers the **custom Python function tools via the data
plane**, applies the **skills via the data plane** (each `skills/<name>/SKILL.md` carries a `tools:`
frontmatter list, so tools show selected in the portal — tools are registered *before* the skills
that reference them), and sets the incident filter and knowledge against the target agent. Remote MCP
references must use bare registered tool names such as `pagerduty_manage_incidents`, not
connector-qualified names such as `pagerduty/pagerduty_manage_incidents`; qualified names appear
selected in the portal but do not resolve in incident execution. ServiceNow is wired as
**change management only** — through the custom Python function tools, not as an incident connector
(incident management is PagerDuty's job).
The script obtains a scoped Entra token from Azure CLI and calls the supported SRE Agent data-plane
routes directly. Re-running is idempotent.
