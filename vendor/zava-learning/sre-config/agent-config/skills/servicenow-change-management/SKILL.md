...
- Credentials (`SERVICENOW_URL/USER/PASS`) are substituted from the local gitignored `.env` only
  while the tool manifest is applied; never request them during an incident or commit them.
