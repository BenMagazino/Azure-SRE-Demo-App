...
`pagerduty_get_incident` if needed). Capture the incident's `html_url` from the
`pagerduty_get_incident` response — that is the operator-facing PagerDuty link the
...
As soon as you start working the incident, call `pagerduty_manage_incidents` with the triggering
incident id in `incident_ids` and `status: "acknowledged"` so responders see it
is owned and stop escalating. This must be a REAL tool call that returns success — do not write
"acknowledged" unless `pagerduty_manage_incidents` actually executed in this thread. If that tool
...
plain-text note with **`Status: Mitigated`** via `pagerduty_add_note_to_incident`, summarising the live
...
Use `pagerduty_add_note_to_incident` to add a concise, factual note. **PagerDuty notes are PLAIN TEXT
...
Call `pagerduty_manage_incidents` with the triggering incident id in `incident_ids` and
`status: "resolved"` to close the incident **only once the incident is already MITIGATED,
