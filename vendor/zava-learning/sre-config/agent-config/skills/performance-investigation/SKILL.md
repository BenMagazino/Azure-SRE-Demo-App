...
3. Optionally confirm against the database itself with a read-only query (connection recipe in the
   knowledge base — run the query inside the reporting VM via `az vm run-command invoke`; the VM's
   managed identity privately retrieves `db-password` and invokes `psql` without returning the secret):
