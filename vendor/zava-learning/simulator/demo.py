...
SCENARIOS = {
    ...
    "app": {
        ...
        "symptom_alert": "Zava-quiz-content-unavailable",
        "probe_path": "/quiz/BIO-101",
    ...
    "query": {
        ...
        "symptom_alert": "Zava-quiz-loading-latency-elevated",
        "probe_path": "/quiz/BIO-101",
        "agent": "performance-agent",
        "slow_threshold_ms": 800,
    ...
    "secret": {
        ...
        "symptom_alert": "Zava-quiz-launch-errors-elevated",
        "probe_path": "/quiz/BIO-101",
        "agent": "connectivity-triage-agent",
...
def poll_agent_thread(cfg: dict, since: datetime, expected_title: str | None = None) -> bool:
    """True if the SRE Agent data plane has a matching recent incident thread."""
    return bool(resolve_incident_thread(cfg, since, expected_title=expected_title))
...
def _dataplane_token():
    out = _az("az account get-access-token "
              "--scope 59f0a04a-b322-4310-adc9-39ac41e9631e/.default "
...
def monitor(cfg: dict, sc: dict, auto_fix: bool):
    ...
    with Live(build(), console=console, refresh_per_second=4, screen=False) as live:
        for tick in range(900):  # ~15 min max
            ...
            # In dev stand-in mode the synthetic journey itself is the detector. Do not wait for
            # an Azure Monitor signal that edge blackholes cannot emit, or for optional PagerDuty
            # credentials, before applying the requested automatic remediation.
            if auto_fix and impacted_seen and not alert_fired \
                    and not cfg["pagerduty"].get("api_token"):
                alert_fired = True
                timeline.add("Synthetic journey detected the outage.", "yellow")
            ...
            if (alert_fired or pd_created) and not agent_seen and cfg.get("agent_name") \
                ...
                last_agent = now
                if poll_agent_thread(cfg, sim_start, sc.get("pd_title")):
                    agent_seen = True
...
def _report(sc, alert_fired, pd_created, agent_seen, recovered, agent_resolved,
    ...
    if success and require_agent:
        ...
        console.print("[green]✔ Service recovered (dev stand-in mode — automated remediation "
                      "substituted for the live SRE Agent).[/]")
...
# ── Preflight + menu ────────────────────────────────────────────────
def preflight(cfg: dict, scenario: str | None = None) -> bool:
    console.print("[bold]Pre-flight checks[/]")
    ok = True
    requires_web = scenario is None or SCENARIOS[scenario].get("web_health", True)
    if requires_web:
        ...
        vm = _az(
            f'az vm list -g "{rg}" '
            '--query "[?starts_with(name, \'vm-zava-reporting-\')].name | [0]" -o tsv'
        ) if rg else None
        if not vm or not vm.strip():
            console.print("  [red]✗ Reporting-worker VM not found for this scenario.[/]")
            ok = False
        else:
            console.print(f"  ✅ reporting-worker VM {vm.strip()}")
