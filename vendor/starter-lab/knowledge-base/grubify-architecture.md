...
This app (this Tauri onboarding tool) automates the above via the "Break Cart" button, which
issues repeated POST requests to this endpoint from the local machine.
...
|---------|----------|------------|
| Memory pressure | Slow responses, OOM restarts | Undersized container (1Gi), memory leak in cart API |
